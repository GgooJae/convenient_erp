sap.ui.define([
    ...
    return Controller.extend("convenienterp.controller.Stock_Order_History_HO", {
        onApplyFilter: function () {
            const oTable = this.byId("orderHistoryTableHO"); // ✔ ID 수정
            const oBinding = oTable.getBinding("rows");

            let aFilters = [];

            // 품목명 필터
            const sOrderItemName = this.byId("orderItemNameFilterHO").getValue().padEnd(20, ' ');
            if (sOrderItemName) {
                aFilters.push(new sap.ui.model.Filter("OrderItemName", sap.ui.model.FilterOperator.Contains, sOrderItemName));
            }

            // 주문 날짜 필터
            const oDateRange = this.byId("orderDateFilterHO").getDateValue();
            const oEndDate = this.byId("orderDateFilterHO").getSecondDateValue();
            ...
            // 주문자 필터
            const sOrderOwner = this.byId("orderOwnerFilterHO").getValue().padEnd(10, ' ');
            if (sOrderOwner) {
                aFilters.push(new sap.ui.model.Filter("OrderOwner", sap.ui.model.FilterOperator.Contains, sOrderOwner));
            }

            // 수량 필터
            const sMinQuantity = this.byId("quantityMinFilterHO").getValue();
            const sMaxQuantity = this.byId("quantityMaxFilterHO").getValue();
            ...
            // 상태 필터
            const sStatus = this.byId("orderStatusFilterHO").getValue().padEnd(10, ' ');
            if (sStatus) {
        ...
        onApprovedPress: function (oEvent) {
            ...
            // 사용자에게 취소 여부를 확인합니다.
            MessageBox.confirm("정말로 해당 주문을 승인하시겠습니까?", {
                title: "주문 승인 확인",
                    ...
                    if (sAction === MessageBox.Action.OK) {
                        ...
                        oModel.update(sPath, oPayload, {
                            success: (oResponseData) => {
                                ...
                                MessageToast.show("주문 상태가 '승인'로 업데이트되었습니다.");
                                // 알람 생성 (취소)
                                const sOwner = sessionStorage.getItem("username");
                                const qty = oData.OrderItemQuan || 1;
                                const arlam = {
                                    UserId: sOwner,
                                    Content: `${sOwner}님이 ${oData.OrderItemName} ${qty}개 주문을 승인`
                                };
